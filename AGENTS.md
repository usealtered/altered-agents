...
- ADD mode means Agent Driven Development: the agent operates directly on root branches in this repo (`main` and `job-*`).

- ADD write scope is strictly `main` and `job-*` only. Never write to any other branch prefix.

- Use `main` for cohesive direct commits. Use `job-*` branches for isolated work, then merge back into `main`.

- The agent has permission to override, replace, restructure, or delete anything in the codebase when operating inside ADD mode and inside the allowed branch scope.
...
- In ADD workflows, always load environment variables explicitly from `.env.agents` (never rely on implicit `.env` loading).
